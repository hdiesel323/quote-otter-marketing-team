...
export interface PhoneRevealrResponse {
  success: boolean;
  data?: {
    phone: string;
    formatted: string;
    valid: boolean;
    line_type: 'mobile' | 'landline' | 'voip' | 'invalid' | 'unknown';
    carrier: string;
    country: string;
    is_voip: boolean;
    is_prepaid?: boolean;
    risk_score?: number; // 0-100, lower = riskier
  };
  error?: string;
}
...
export interface ValidationResult {
  ...
  reason: string;
  phoneRevealrData?: PhoneRevealrResponse['data'];
}

export class PhoneRevealrClient {
   ...
   */
  async validatePhone(phoneNumber: string): Promise<ValidationResult> {
   ...
   */
  async validateBatch(phoneNumbers: string[]): Promise<Map<string, ValidationResult>> {
   ...
   * Call PhoneRevealr API
   */
  private async callPhoneRevealrAPI(phone: string): Promise<PhoneRevealrResponse> {
   ...
   */
  private analyzePhoneData(response: PhoneRevealrResponse): ValidationResult {
   ...
   */
  private fallbackValidation(phone: string): ValidationResult {
 ...
 * Singleton instance
 */
export const phoneRevealr = new PhoneRevealrClient();

/**
 * Express middleware for phone validation
 */
export function validatePhoneMiddleware(phoneField: string = 'phone') {
  return async (req: any, res: any, next: any) => {
    ...
    try {
      const validation = await phoneRevealr.validatePhone(phone);
